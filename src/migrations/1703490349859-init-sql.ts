...
export class InitSql1703490349859 implements MigrationInterface {
  name = 'InitSql1703490349859';

  public async up(queryRunner: QueryRunner): Promise<void> {
    await queryRunner.query(
      `CREATE TABLE \`permissions\` (\`id\` int NOT NULL AUTO_INCREMENT, \`code\` varchar(20) NOT NULL COMMENT 'permission code', \`description\` varchar(100) NOT NULL COMMENT 'permission description', PRIMARY KEY (\`id\`)) ENGINE=InnoDB`,
    );
    await queryRunner.query(
      `CREATE TABLE \`roles\` (\`id\` int NOT NULL AUTO_INCREMENT, \`name\` varchar(20) NOT NULL COMMENT 'role name', PRIMARY KEY (\`id\`)) ENGINE=InnoDB`,
    );
    await queryRunner.query(
      `CREATE TABLE \`users\` (\`id\` int NOT NULL AUTO_INCREMENT, \`username\` varchar(50) NOT NULL COMMENT 'user name', \`password\` varchar(50) NOT NULL COMMENT 'password', \`nick_name\` varchar(50) NOT NULL COMMENT 'nickname', \`email\` varchar(70) NOT NULL COMMENT 'email', \`avatar\` varchar(150) NULL COMMENT 'avatar', \`phoneNumber\` varchar(20) NULL COMMENT 'phone number', \`isFrozen\` tinyint NOT NULL COMMENT 'is frozen' DEFAULT 0, \`isAdmin\` tinyint NOT NULL COMMENT 'is adamin' DEFAULT 0, \`createAt\` datetime(6) NOT NULL DEFAULT CURRENT_TIMESTAMP(6), \`updateAt\` datetime(6) NOT NULL DEFAULT CURRENT_TIMESTAMP(6) ON UPDATE CURRENT_TIMESTAMP(6), PRIMARY KEY (\`id\`)) ENGINE=InnoDB`,
    );
    await queryRunner.query(
